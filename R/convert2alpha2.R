...
#' @param alpha2 Vector of aplha 2 code to be converted
...
#' @examples
#' nationality <- c("England", "Canada")
#' convert2alpha2(nationality)
#' @export
#'
convert2alpha2 <- function(country, nationality, alpha3, UN_code) {
...
Zimbabwe,Zimbabwean,ZW,ZWE,716"
  ...
  if(!missing(country)){
    result <- ctry_data[ctry_data[['Country']] == country, ]$Alpha2
  }else if(!missing(nationality)){
    result <- ctry_data[ctry_data[['Nationality']] == nationality, ]$Alpha2
  }else if(!missing(alpha3)){
    result <- ctry_data[ctry_data[['Alpha3']] == alpha3, ]$Alpha2
  }else if(!missing(UN_code)){
    result <- ctry_data[ctry_data[['UN_code']] == UN_code, ]$Alpha2
  }
