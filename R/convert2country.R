...
#' convert2country(nationality)
#' @export
#'
convert2country <- function(nationality, alpha2, alpha3, UN_code) {
...
Zimbabwe,Zimbabwean,ZW,ZWE,716"
  ...
  if(!missing(nationality)){
    result <- ctry_data[ctry_data[['Nationality']] == nationality, ]$Country
  }else if(!missing(alpha2)){
    result <- ctry_data[ctry_data[['Alpha2']] == alpha2, ]$Country
  }else if(!missing(alpha3)){
    result <- ctry_data[ctry_data[['Alpha3']] == alpha3, ]$Country
  }else if(!missing(UN_code)){
    result <- ctry_data[ctry_data[['UN_code']] == UN_code, ]$Country
  }
