...
#' convert2alpha3(nationality)
#' @export
#'
convert2alpha3 <- function(country, nationality, alpha2, UN_code) {
...
Zimbabwe,Zimbabwean,ZW,ZWE,716"
  ...
  if(!missing(country)){
    result <- ctry_data[ctry_data[['Country']] == country, ]$Alpha3
  }else if(!missing(nationality)){
    result <- ctry_data[ctry_data[['Nationality']] == nationality, ]$Alpha3
  }else if(!missing(alpha2)){
    result <- ctry_data[ctry_data[['Alpha2']] == alpha2, ]$Alpha3
  }else if(!missing(UN_code)){
    result <- ctry_data[ctry_data[['UN_code']] == UN_code, ]$Alpha3
  }
