...
#' @examples
#' country <- c("Canada", "England")
#' convert2nationality(country)
#' @export
#'
convert2nationality <- function(country, alpha2, alpha3, UN_code) {
...
Zimbabwe,Zimbabwean,ZW,ZWE,716"
  ...
  if(!missing(country)){
    result <- ctry_data[ctry_data[['Country']] == country, ]$Nationality
  }else if(!missing(alpha2)){
    result <- ctry_data[ctry_data[['Alpha2']] == alpha2, ]$Nationality
  }else if(!missing(alpha3)){
    result <- ctry_data[ctry_data[['Alpha3']] == alpha3, ]$Nationality
  }else if(!missing(UN_code)){
    result <- ctry_data[ctry_data[['UN_code']] == UN_code, ]$Nationality
  }
